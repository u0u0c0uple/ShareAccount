...
const HeaderComponent = () => {
  ...
  return (
    <div className="flex bg-slate-50 w-screen h-28 fixed drop-shadow-xl z-9">
      {/* section 1 */}
      <div className="flex w-1/2 h-full basis-1/2">
        {/* Logo */}
        <div className="w-1/3 h-full basis-1/3 relative">
          <img src="./src/assets/image/logo_icon_80.png" alt="logo"
        className="absolute ml-12 top-1/2 -translate-y-1/2" />
        </div>
        <div className="flex w-2/3 h-full basis-2/3 justify-between">
          <a className={`my-auto text-xl font-semibold`} onClick={() => navigate('/account')}>가계부</a>
          <a className={`my-auto text-xl `} onClick={() => navigate('/')}>일기</a>
          <a className={`my-auto text-xl `} onClick={() => navigate('/account')}>기프티콘</a>
          <a className={`my-auto text-xl `} onClick={() => navigate('/account')}>우리</a>
        </div>
      </div>
      {/* section 2 */}
